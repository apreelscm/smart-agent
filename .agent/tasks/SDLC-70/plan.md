# SDLC-70: na ekranie ofert, zmien przycisk Wyczyść na Clear

## Specification

### Overview
This task updates the clear-filters button label on the offers screen from `Wyczyść` to `Clear` without changing the button’s placement, styling, enabled/disabled logic, or clear-all behavior. The scope is limited to the offers list UI and its regression coverage.

### User Stories
- US-001: As a sales agent, I want to see the clear-filters button labeled `Clear` on the offers screen, so that the wording matches the expected interface vocabulary.
  - Given I open the offers screen
  - When the toolbar is rendered
  - Then the clear-filters button is displayed with the label `Clear`

- US-002: As a sales agent, I want the `Clear` button to keep resetting all active filters, so that I can quickly return to the default offers list view.
...
- FR-001: Change the offers-screen clear-filters button label from `Wyczyść` to `Clear`.
...
### Edge Cases
- EC-001: When no filters are active, the `Clear` button remains disabled exactly as today.
- EC-002: When the search filter produces an empty state, clicking `Clear` restores the default results list and count.
- EC-003: When the page is opened through the root redirect (`/` to `/offers`), the button still renders with the label `Clear`.
- EC-004: Other Polish labels on the offers page, including empty-state text and action menus, remain unchanged.

### Success Criteria
- [ ] On the offers screen, the clear-filters button label is `Clear`
- [ ] The previous label `Wyczyść` no longer appears for that button
- [ ] Clicking `Clear` still resets search, status, product, sort field, and sort direction to defaults
...
1. Update the button `label` in `offers-home-page.component.html` from `Wyczyść` to `Clear`.
...
3. Update the existing Playwright tests in `tests/e2e/offers-filters.spec.ts` to assert `Clear` instead of `Wyczyść` while preserving the behavioral regression check.
...
| 1.2 | Change the visible button label | `src/app/features/offers/pages/offers-home-page.component.html` | Replace `label="Wyczyść"` with `label="Clear"` without changing the click handler, class, inline style, or disabled binding. |
...
| # | Task | Files | Description |
|---|------|-------|-------------|
| 2.1 | Update label assertion in e2e test | `tests/e2e/offers-filters.spec.ts` | Change selectors and assertions from `Wyczyść` to `Clear` in the label-verification scenario. |
...
|------|--------|-------------|
| `src/app/features/offers/pages/offers-home-page.component.html` | MODIFY | Update the offers clear-filters button label from `Wyczyść` to `Clear`. |
...
6. [ ] Manual check on `/offers` confirms the button shows `Clear` and still resets filters correctly