...
- FR-001: Add a presentation-currency control to `src/app/features/offers/pages/offers-home-page.component.html` with selectable values `EUR` and `USD`.
...
### Success Criteria
- [ ] The offers page shows a currency selector with EUR and USD options.
...
4. **Update the template and styles**
   - Add a currency selector in the existing toolbar next to other filters, reusing the current PrimeNG `p-select` pattern already used for status/product/sort controls.
   ...
   - Add minimal styling in `offers-home-page.component.scss` for the new selector and for any small helper/error text.
...
| `src/app/features/offers/pages/offers-home-page.component.html` | MODIFY | Add EUR/USD selector and replace hardcoded PLN premium rendering with selected-currency rendering. |
| `src/app/features/offers/pages/offers-home-page.component.scss` | MODIFY | Style the new selector and helper/error text in the existing toolbar layout. |
...
4. [ ] The offers toolbar shows EUR/USD currency selection
