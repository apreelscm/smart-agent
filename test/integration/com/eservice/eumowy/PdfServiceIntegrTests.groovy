...
class PdfServiceIntegrTests extends GroovyTestCase {
	...
	static URL url = new PdfServiceIntegrTests().getClass().getResource("PdfServiceIntegrTests.class");
	static String fileTemplatePath = File.separator+"otherResources" +File.separator+"pdf_templates" + File.separator;
	static String fileTemplateOutPath = File.separator+"target" +File.separator+ "pdf_out" + File.separator;
	...
	void process(templateName, outName, data){
		byte[] pdf = pdfService.fillPdfFormFromURI(getTemplatePath()+templateName, data, PdfService.FontType.ARIAL)
